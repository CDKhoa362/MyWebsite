    ...
    public class DistrictController : Controller
    {
        ...
        private async Task<List<SelectListItem>> _getStateSelectList()
        {
            var stateSelectionList = await _db.States.Select(s => new SelectListItem
                                                                      {Text = s.StateName,
                                                                       Value=s.StateId.ToString()}).ToListAsync();
            return stateSelectionList;
        }

        public async Task<IActionResult> Index(string searchString, int page = 1, int pageSize = 30)
        {
            // Paginition
            ...
            // Search
            IQueryable<District> query = _db.Districts.Include(d => d.State);
            if (!string.IsNullOrEmpty(searchString))
            {
                query = query.Where(w => w.DistrictId.Contains(searchString) ||
                                    w.DistrictName.Contains(searchString) || 
                                   ( w.State != null && w.State.StateName.Contains(searchString)));
            }
